...
export default function Navbar({ leagues, selectedLeague, onLeagueChange }) {
    const { isAuthenticated, redirectToLogin, logout, isLoading } = useAuth();
            ...
            <div className="max-w-6xl mx-auto px-4 py-3 sm:py-0 sm:h-16 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-0">
                ...
                <div className="flex items-center justify-between w-full sm:w-auto gap-2">
                    ...
                    <div className="flex sm:hidden items-center gap-2 shrink-0">
                        <button
                            onClick={isAuthenticated ? logout : redirectToLogin}
                            disabled={isLoading}
                            className={`text-xs font-semibold px-3 py-1.5 rounded-lg transition-all whitespace-nowrap ${isLoading ? 'opacity-50 cursor-not-allowed' : ''
                                } ${isAuthenticated ? 'bg-rose-600 hover:bg-rose-700' : 'bg-indigo-600 hover:bg-indigo-700'}`}
                        >
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto justify-center sm:justify-end">
                    ...
                    <div className="hidden sm:flex items-center justify-center gap-3 shrink-0">
                        ...
                        <button
                            onClick={isAuthenticated ? logout : redirectToLogin}
                            disabled={isLoading}
                            className={`text-xs font-semibold px-4 py-1.5 rounded-lg transition-all shadow-sm whitespace-nowrap ${isLoading ? 'opacity-50 cursor-not-allowed' : ''
                                } ${isAuthenticated ? 'bg-rose-600 hover:bg-rose-700' : 'bg-indigo-600 hover:bg-indigo-700'}`}
                        >
                            {isLoading ? 'Loading...' : (isAuthenticated ? 'Logout' : '🔐 Admin Login')}
                        </button>
                    </div>
                </div>
            </div>
